...
const pickMistery = () => {
  ...
  const misteryEnvelope = [characterCard, weaponCard, roomCard];
  return misteryEnvelope;
}

// Reveal the mistery
const revealMistery = (misteryEnvelope) => {
  const firstName = misteryEnvelope[0].first_name;
  const lastName = misteryEnvelope[0].last_name;
  const weapon = misteryEnvelope[1].name;
  const place = misteryEnvelope[2].name;
